#!/usr/bin/env python3
"""
Seed Svensk Ragg Examples - RetICL Implementation

Creates a ChromaDB collection 'svensk_ragg_examples' with few-shot examples
...
Usage:
    python seed_svensk_ragg_examples.py
"""
...
def seed_svensk_ragg_examples():
    """
    Seed ChromaDB collection with Svensk Ragg examples.

    Creates 'svensk_ragg_examples' collection and indexes examples
    ...
    try:
        ...
        collection = client.create_collection(
            ...
            metadata={"description": "Svensk Ragg few-shot examples for RetICL"},
        )
        logger.info(f"Created new collection: {collection_name}")

    # Process examples
    logger.info(f"Processing {len(SVENSK_RAGG_EXAMPLES)} examples")

    ids = []
    documents = []  # User questions for embedding
    metadatas = []  # Full example JSON

    for i, example in enumerate(SVENSK_RAGG_EXAMPLES):
    ...
    logger.info(f"✅ Successfully seeded {final_count} Svensk Ragg examples")
...
if __name__ == "__main__":
    result = seed_svensk_ragg_examples()
