#!/usr/bin/env python3
"""
RIKSBANKEN SCRAPER AND INDEXER
Complete workflow: Scrape + Index to ChromaDB
"""
...
class RiksbankenScraper:
    ...
    def index_publications(self):
        """Index collected publications to ChromaDB"""
        if not self.publications:
            return

        print(f"\nIndexing {len(self.publications)} publications to ChromaDB...")

        batch_size = 10
        for i in range(0, len(self.publications), batch_size):
            batch = self.publications[i : i + batch_size]
            ...
            try:
                ...
                print(f"  Indexed {self.docs_indexed}/{len(self.publications)}...")
    ...
    def run(self) -> dict:
        """Main workflow"""
        print("=" * 60)
        print("RIKSBANKEN SCRAPER AND INDEXER")
        print("=" * 60)
...
def main():
    ...
    # Save results
    output_file = f"/home/ai-server/AN-FOR-NO-ASSHOLES/09_CONSTITUTIONAL-AI/riksbanken_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
