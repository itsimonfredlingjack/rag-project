...
// Svensk Ragg public streaming endpoint.
const API_ENDPOINT = `${BACKEND_URL}/api/svensk-ragg/agent/query/stream`;
...
export type Source = QueryResultSource;
...
function normalizeSources(data: unknown): QueryResultSource[] {
  ...
  return data.map((s: Record<string, unknown>) => ({
    id: String(s?.id ?? ""),
    document_id:
      s?.document_id != null ? String(s.document_id) : undefined,
    ...
    source_scope:
      s?.source_scope != null ? String(s.source_scope) : undefined,
    date: s?.date != null ? String(s.date) : undefined,
    url: s?.url != null ? String(s.url) : undefined,
    retriever:
      s?.retriever != null ? String(s.retriever) : undefined,
    retriever_source:
      s?.retriever_source != null ? String(s.retriever_source) : undefined,
  }));
}

function normalizeCitations(data: unknown): QueryResultCitation[] {
  if (!Array.isArray(data)) return [];
  return data.map((citation: Record<string, unknown>) => ({
    claim: String(citation?.claim ?? ""),
    source_id: String(citation?.source_id ?? ""),
    source_title: String(citation?.source_title ?? ""),
    source_collection: String(citation?.source_collection ?? ""),
    tier: String(citation?.tier ?? ""),
  }));
}

function normalizeRetrieval(data: unknown): QueryResultRetrievalMetadata | null {
  if (!data || typeof data !== "object" || Array.isArray(data)) return null;
  return data as QueryResultRetrievalMetadata;
}

async function formatBackendError(response: Response): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      return message ? `${fallback}: ${String(message)}` : `${fallback}: ${body}`;
    } catch {
  ...
  fetchFacets: async () => {
    try {
      ...
      set({
        availableFacets: {
          agencies: [
            { id: "riksdagen", name: "Sveriges riksdag / öppna data" }
          ],
          doc_types: [
            { id: "riksdag", name: "Riksdagsdokument" }
          ],
  ...
  startSearch: async (mode = "auto", uiMode) => {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (!response.ok) throw new Error(await formatBackendError(response));
        ...
        const processEventData = (rawData: string) => {
          try {
            const data = JSON.parse(rawData);
              ...
              switch (data.type) {
                ...
                case "metadata": {
                  ...
                  if (data.mode)
                    get().updateQueryResult(queryResultId, {
                      retrievalStrategy: String(data.mode),
                    });
                  if (data.data_source_attribution)
                    get().updateQueryResult(queryResultId, {
                      dataSourceAttribution: String(data.data_source_attribution),
                    });
                  if (data.refusal_reason)
                    get().updateQueryResult(queryResultId, {
                      refusalReason: String(data.refusal_reason),
                      error: null,
                    });
                  ...
                  if (data.retrieval)
                    get().updateQueryResult(queryResultId, {
                      retrieval: normalizeRetrieval(data.retrieval),
                    });
                ...
                case "refusal": {
                  get().updateQueryResult(queryResultId, {
                    refusalReason: data.reason ? String(data.reason) : "unknown",
                    error: null,
                    searchStage: "reading",
                  });
                  break;
                      ...
                      get().updateQueryResult(queryResultId, {
                        ...
                        pipelineLog: [
                          ...active.pipelineLog,
                          {
                            ts: Date.now(),
                            stage: "idle",
                            message: `Error: ${data.message ?? data.error ?? "unknown"}`,
                          },
                        ].slice(-MAX),
                      });
                  }
            ...
            throw new Error("Malformed streaming event from backend");
          }
        };

        const processParsedEvents = (events: { data: string }[]) => {
        ...
        while (true) {
          ...
          processParsedEvents(sseParser.push(chunk));
        }

        if (get().currentSearchId === searchId) {
          const finalChunk = decoder.decode();
          if (finalChunk) processParsedEvents(sseParser.push(finalChunk));
          processParsedEvents(sseParser.flush());
        }

        if (
          get().currentSearchId === searchId &&
          get().isSearching &&
          !sawDone
        ) {
          const active = get().queries.find((q) => q.id === queryResultId);
