...
export function QueryBar({
  ...
  const submittedQuery =
    queryText ?? queries.map((q) => q.query)[queries.length - 1] ?? "";
  ...
  return (
    <div className={clsx("w-full", className)}>
      <div className="flex items-center justify-between gap-4">
        ...
        >
          ...
          <button
            ...
            disabled={!String(submittedQuery).trim() || isSearching}
            className={clsx(
              ...
              (!String(submittedQuery).trim() || isSearching) &&
                "opacity-50 cursor-not-allowed hover:bg-teal-50",
            )}
            title="Run again"
          >
      ...
      {isEditing && recentQueries.length > 0 && (
