...
export const QueryProcessor: React.FC<{ className?: string }> = ({
  className,
}) => {
  ...
  const activeQuery = useMemo(
    () => (activeQueryId ? queries.find((q) => q.id === activeQueryId) : null),
    [queries, activeQueryId],
  );
  const pipelineStage = activeQuery?.pipelineStage;
  const retrievalStrategy = activeQuery?.retrievalStrategy ?? null;
  const searchStage = activeQuery?.searchStage;
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4 text-xs text-slate-400 font-mono">
          <span className="flex items-center gap-1">
            <Clock className="w-3 h-3" strokeWidth={1.5} /> {elapsed.toFixed(1)}
            s
          </span>
          <span className="flex items-center gap-1">
        ...
        <div className="space-y-3 md:border-l md:border-slate-200 md:pl-6">
          ...
          <div className="space-y-2">
            <div className="py-2 px-3 bg-slate-50 rounded border border-slate-200">
              <span className="block text-[10px] text-slate-500 mb-1.5 uppercase tracking-wide">
                Retrieval Mode
              </span>
