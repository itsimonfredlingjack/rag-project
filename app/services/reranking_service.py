...
from ..core.exceptions import RerankingError, ServiceNotInitializedError
...
class RerankingService(BaseService):
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            self.logger.info("BGE reranker model loaded (~1.2GB VRAM)")
    ...
    async def rerank(
        ...
        try:
            ...
            self.logger.info(
                f"Reranking complete: {len(reranked_docs)} docs in {latency_ms:.1f}ms (top: {reranked_scores[0]:.4f if reranked_scores else 0:.4f})"
            )
