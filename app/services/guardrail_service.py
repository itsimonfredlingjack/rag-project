...
class GuardrailService(BaseService):
    ...
    def apply_corrections(self, text: str) -> GuardrailResult:
        ...
        for original, pattern_data in self._term_patterns.items():
            ...
            if count > 0:
                ...
                if count > 5:
                    self.logger.warning(
                        f"Many outdated terms detected: {original} ({count} times)"
                    )
    ...
    def validate_citations(self, text: str) -> Tuple[bool, List[str]]:
        ...
        for pattern in self._citation_patterns:
            ...
            for match in matches:
                ...
                if not re.search(r'\[Källa\s+\d+\s', match) and \
                   not re.search(r'\]\s', match):
    ...
    def validate_response(
        ...
        if mode.lower() == "evidence":
            ...
            if not is_valid:
                ...
                for issue in citation_issues:
                    corrections_result.corrections.append(Correction(
        ...
        corrections_result.evidence_level = self._determine_evidence_level_from_text(
            corrected_text
        )
