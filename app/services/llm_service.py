...
class LLMService(BaseService):
        ...
        if self._client is None:
            self._client = httpx.AsyncClient(
                ...
                timeout=httpx.Timeout(
                    ...
                    write=30.0,
                    pool=self._config.pool_connections,
                ),
                limits=httpx.Limits(
    ...
    async def chat_stream(
        ...
        stats = StreamStats(start_time=0.0)  # Will be set on first token
    ...
    async def chat_with_fallback(
        ...
        try:
            ...
            # Try fallback model
            self.logger.warning(f"Primary model timed out, trying fallback: {self._config.fallback_model}")
            yield f"fallback:primary→{self._config.fallback_model}", None
            
            try:
                async for token, stats in self.chat_stream(messages, self._config.fallback_model, config_override):
                    if token:
                        yield f"token:{token}", stats
                    else:
                        yield "done", stats
