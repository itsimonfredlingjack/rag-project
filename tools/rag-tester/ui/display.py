 ...
 [magenta]╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝[/]     [cyan]╚══════╝  ╚═══╝  ╚═╝  ╚═╝╚══════╝[/]
        ...
        grid.add_row(
            ...
            make_stat_panel(
                ...
                "bright_magenta",
                "📈",
                subtitle_text=Text.from_markup(sparkline or "No Data"),
            ),
...
class StreamRenderer:
    ...
    def __init__(self, console: Console, question: str, expected: str | None = None):
        ...
        self.live = Live(
            self.layout, console=self.console, refresh_per_second=12, screen=False, transient=False
        )
    ...
    def _update_display(self):
        ...
        self.layout["status"].update(
            Columns([Align.left(pipeline), Align.right(Text(stats_text, style="dim white"))])
        )
...
class HistoryView:
    ...
    def render(self, history: list[dict]):
        ...
        table = Table(
            title="[bold magenta]Operational Logs[/]",
            box=HEAVY_EDGE,
            expand=True,
            border_style="magenta",
        )
    ...
    async def show(self, sources: list[dict]):
        ...
        while True:
            ...
            content_display = (
                Markdown(content)
                if content
                else Text("No content available", style="italic dim red")
            )
            ...
            controls.add_row(
                "[bold white]← Prev (P)[/]   [bold white]Next (N) →[/]   [bold red]Exit (Q)[/]"
            )
